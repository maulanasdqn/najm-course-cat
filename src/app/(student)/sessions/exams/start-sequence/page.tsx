...
export const Component: FC = (): ReactElement => {
    ...
    answerExamMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          if (testQuery.data?.data.next_test_id) {
            navigate(
              `/student/sessions/${params.sessionId}/exams/${testQuery.data?.data.next_test_id}/start-sequence`,
              {
                replace: true,
              },
            );
            toast.success("Ujian " + testQuery.data?.data.test_name + " telah disimpan.");
            return;
          }
          finishExam();
        },
  ...
  const nextQuestion = () => {
    ...
    if (currentQuestion < testQuery.data.data.questions.length - 1) {
      navigate(
        `/student/sessions/${params.sessionId}/exams/${params.examId}/start-sequence?page=${currentQuestion + 2}`,
      );
    }
  };
  ...
  const prevQuestion = () => {
    if (currentQuestion > 0) {
      navigate(
        `/student/sessions/${params.sessionId}/exams/${params.examId}/start-sequence?page=${currentQuestion}`,
      );
    }
  };
  ...
  const goToQuestion = (index: number) => {
    navigate(
      `/student/sessions/${params.sessionId}/exams/${params.examId}/start-sequence?page=${index + 1}`,
    );
  };
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <h2 className="text-xl font-bold text-gray-700">
              Test {testQuery.data?.data.test_name}
            </h2>
