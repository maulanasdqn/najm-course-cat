...
export const Component: FC = (): ReactElement => {
  ...
  const [activeTab, setActiveTab] = useState<keyof typeof exams>("kognitif");
      ...
      <section className="flex flex-1 w-full max-w-7xl">
        <main className="flex-1 flex flex-col gap-4 p-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center">
                      <span className="text-2xl font-bold text-green-600">
                        {exams[activeTab].correctAnswers}
                      </span>
                      <span className="text-gray-500 ml-1">
                        / {exams[activeTab].totalQuestions}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="relative w-32 h-32">
                <svg className="w-full h-full" viewBox="0 0 100 100">
                  ...
                  <circle
                    ...
                    r="45"
                    fill="none"
                    stroke={finalScore >= 60 ? "#4ade80" : finalScore >= 40 ? "#facc15" : "#ef4444"}
                    strokeWidth="8"
                    strokeLinecap="round"
                    strokeDasharray="283"
                  ...
                  <text
                    ...
                    fontSize="24"
                    fontWeight="bold"
                    fill={finalScore >= 60 ? "#16a34a" : finalScore >= 40 ? "#ca8a04" : "#dc2626"}
                  >
                    {finalScore}
                  </text>
                  <text
          ...
          <Tabs
            items={items}
            activeTab={activeTab}
            onTabChange={(val) => setActiveTab(val as keyof typeof exams)}
          />
        </main>
      </section>
    </div>
...
type QuestionsReviewTabProps = {
  questions: TTestDetailResponse["data"]["questions"];
  answers: TExamAnswerRequest["questions"];
};

const QuestionsReviewTab = (props: QuestionsReviewTabProps) => {
  return (
    <>
      <h3 className="text-xl font-semibold text-gray-800 mb-4">Ulasan Pertanyaan</h3>
      ...
      {props.questions.map((question, index) => {
        ...
        const selectedOptionId = props.answers[index]?.option_id;
